...
async function getWeatherForOneDay(lat, lon) {
  const weatherForToday = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${APP_ID}&units=metric`;

  try {
    const response = await fetch(weatherForToday);
    const data = await response.json();

    // выводим информацию о погоде на 1 день
    console.log('Прогноз погоды на 1 день:');
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
// ---------------   Функция для получения прогноза погоды на 5 день  ---------------  //

async function getWeatherForFiveDays(lat, lon) {
  ...
  try {
    ...
    const data = await response.json();
    // выводим информацию о погоде на 5 дней
    console.log('Прогноз погоды на 5 дней:');
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
navigator.geolocation.getCurrentPosition(
  (position) => {
    ...
    // вызываем функции для получения прогноза погоды
    getWeatherForOneDay(lat, lon);
    getWeatherForFiveDays(lat, lon);
  },
  (error) => {
